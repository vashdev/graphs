...
public class GraphAdjList extends Graph {
	...
	//	Neighbors.addAll(adjListsMap.get(v));
	//	for(int x : Neighbors) System.out.println("out deg"+Integer.toString(v)+ " :" + Integer.toString(x));
	//	return Neighbors;
	...
	public List<Integer> getInNeighbors(int v) {
		...
		for (int u : adjListsMap.keySet()) {
			...
			for (int w : adjListsMap.get(u)) {
				if (v == w) {
					inNeighbors.add(u);
				//	System.out.println("in deg"+Integer.toString(v)+ " :" + Integer.toString(w));
				}
			}
		}
		return inNeighbors;
	 ...
	 */		
	 public List<Integer> getDistance2(int v) {
		 ...
		 for(int i =0; i < hop1.size(); i++){
			 hop2.addAll(getNeighbors(hop1.get(i)));
		 }
